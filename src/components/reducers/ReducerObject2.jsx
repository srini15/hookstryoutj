    ...
    switch (action.type) {
        ...
        case 'updateLang':
            if (action.checked) {
                ...
                return { ...state, lang: state.lang.filter((value) => value !== action.value) }
            }
            ...
            <div className='flex flex-col gap-5 mx-auto max-w-96 p-5'>
                ...
                <div className='flex flex-col gap-5'>
                    ...
                    <div className='flex border border-solid border-slate-100 gap-3'>
                        <input type='checkbox' onChange={(e) => { dispatch({ type: 'updateLang', value: e.target.value, checked: e.target.checked }) }} value="PHP" />PHP
                        <input type='checkbox' onChange={(e) => { dispatch({ type: 'updateLang', value: e.target.value, checked: e.target.checked }) }} value="Java" />Java
                        <input type='checkbox' onChange={(e) => { dispatch({ type: 'updateLang', value: e.target.value, checked: e.target.checked }) }} value="React" />React
                        <input type='checkbox' onChange={(e) => { dispatch({ type: 'updateLang', value: e.target.value, checked: e.target.checked }) }} value="CSharp" />C Sharp
                    </div>
